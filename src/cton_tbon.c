...
static void cton_serialize_value(cton_ctx *ctx, cton_buf *buf, cton_obj *obj)
{
	cton_bool *b;
...
static int cton_serialize_string(cton_ctx *ctx, cton_buf *buf, cton_obj *obj)
{
	...
	cton_serialize_64bit(ctx, buf, (void *)&length);
	...
	size_t *index, uint8_t *ptr, size_t len, cton_type type)
{
	...
	str_len = cton_deserialize_64bit(ptr + *index);
	*index += 8;
...
cton_deserialize_hash(cton_ctx *ctx, size_t *index, uint8_t *ptr, size_t len)
{
	...
	hash_len = cton_deserialize_64bit(ptr + *index);
	*index += 8;
...
cton_deserialize_array(cton_ctx *ctx, size_t *index, uint8_t *ptr, size_t len)
{
	...
	*index += 1;
	...
	arr_len = cton_deserialize_64bit(ptr + *index);
	*index += 8;
