...
static uint8_t tbonv1_getid(cton_type type)
{
	switch (type) {
...
static int tbonv1_serialize_object(cton_ctx *ctx, cton_buf *buf, cton_obj *obj);
static int tbonv1_serialize_hash(cton_ctx *ctx, cton_buf *buf, cton_obj *obj);
static int tbonv1_serialize_array(cton_ctx *ctx, cton_buf *buf, cton_obj *obj);
static int tbonv1_serialize_string(cton_ctx *ctx, cton_buf *buf, cton_obj *obj);
static int tbonv1_serialize_binary(cton_ctx *ctx, cton_buf *buf, cton_obj *obj);

static void tbonv1_serialize_8bit(cton_ctx *ctx, cton_buf *buf, void *ptr);
static void tbonv1_serialize_16bit(cton_ctx *ctx, cton_buf *buf, void *ptr);
static void tbonv1_serialize_32bit(cton_ctx *ctx, cton_buf *buf, void *ptr);
static void tbonv1_serialize_64bit(cton_ctx *ctx, cton_buf *buf, void *ptr);
static void tbonv1_serialize_vw(cton_ctx *ctx, cton_buf *buf, uint64_t data);


static void
tbonv1_serialize_8bit(cton_ctx *ctx, cton_buf *buf, void *ptr)
{
	uint8_t data;
...
static void
tbonv1_serialize_16bit(cton_ctx *ctx, cton_buf *buf, void *ptr)
{
	uint16_t data;
...
static void
tbonv1_serialize_32bit(cton_ctx *ctx, cton_buf *buf, void *ptr)
{
	uint32_t data;
...
static void
tbonv1_serialize_64bit(cton_ctx *ctx, cton_buf *buf, void *ptr)
{
	uint64_t data;
...
static void
tbonv1_serialize_vw(cton_ctx *ctx, cton_buf *buf, uint64_t data)
{
	(void)ctx;

	if (data <= 0x7F) {
		/* 7bit, 1 byte */
		cton_buffer_putchar(buf, (data & 0x7F));

	} else if (data <= 0x7FF) {
		/* 11bit, 2 byte */
		cton_buffer_putchar(buf, ( 0xC0 | (data >> 6) ));
		cton_buffer_putchar(buf, ( 0x80 | (data & 0x3F) ));

	} else if (data <= 0xFFFF) {
		/* 3 byte */
		cton_buffer_putchar(buf, ( 0xE0 | (data >> 12) ));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 6) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | (data & 0x3F) ));

	} else if (data <= 0x1FFFFF) {
		/* 4 byte */
		cton_buffer_putchar(buf, ( 0xF0 | (data >> 18) ));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 12) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 6) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | (data & 0x3F) ));

	} else if (data <= 0x3FFFFFF) {
		/* 5 byte */
		cton_buffer_putchar(buf, ( 0xF8 | (data >> 24) ));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 18) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 12) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 6) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | (data & 0x3F) ));
		...
		/* 6 byte */
		cton_buffer_putchar(buf, ( 0xFC | (data >> 30) ));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 24) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 18) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 12) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 6) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | (data & 0x3F) ));
		...
		cton_buffer_putchar(buf, 0xFE);
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 60) & 0x0F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 54) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 48) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 42) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 36) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 30) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 24) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 18) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 12) & 0x3F)));
		cton_buffer_putchar(buf, ( 0x80 | ((data >> 6) & 0x3F)));
...
static void
tbonv1_serialize_value(cton_ctx *ctx, cton_buf *buf, cton_obj *obj)
{
	cton_bool *b;
	cton_type type;
	...
	if (type == CTON_NULL) {
		cton_buffer_putchar(buf, TBON_ID_NULL);

	} else if (type == CTON_BOOL) {
		b = (cton_bool *)cton_object_getvalue(obj);

		if (*b == CTON_TRUE) {
			cton_buffer_putchar(buf, TBON_ID_TRUE);
		} else {
			cton_buffer_putchar(buf, TBON_ID_FALSE);
		}

	} else if (type == CTON_BINARY) {
		tbonv1_serialize_binary(ctx, buf, obj);

	} else if (type == CTON_STRING) {
		tbonv1_serialize_string(ctx, buf, obj);
		
	} else if (type == CTON_ARRAY) {
		tbonv1_serialize_array(ctx, buf, obj);
		
	} else if (type == CTON_HASH) {
		tbonv1_serialize_hash(ctx, buf, obj);
		
	} else if (type == CTON_INT8 || type == CTON_UINT8 || type == CTON_FLOAT8) {
		tbonv1_serialize_8bit(ctx, buf, obj);

	} else if (type == CTON_INT16 || type == CTON_UINT16 || \
		type == CTON_FLOAT16) {
		tbonv1_serialize_16bit(ctx, buf, obj);
		
	} else if (type == CTON_INT32 || type == CTON_UINT32 || \
		type == CTON_FLOAT32) {
		tbonv1_serialize_32bit(ctx, buf, obj);
		
	} else if (type == CTON_INT64 || type == CTON_UINT64 || \
		type == CTON_FLOAT64) {
		tbonv1_serialize_64bit(ctx, buf, obj);
...
static int
tbonv1_serialize_object(cton_ctx *ctx, cton_buf *buf, cton_obj *obj)
{
	uint8_t id;

	id = tbonv1_getid(obj->type);

	if (id != TBON_ID_NULL && id != TBON_ID_TRUE) {
		cton_buffer_putchar(buf, id);
	}

	tbonv1_serialize_value(ctx, buf, obj);
...
cton_obj *
tbonv1_serialize(cton_ctx *ctx, cton_obj *obj)
{
	cton_buf *buf;
	cton_obj *output;
	...
	tbonv1_serialize_object(ctx, buf, obj);
...
static int tbonv1_serialize_string(cton_ctx *ctx, cton_buf *buf, cton_obj *obj)
{
	...
	tbonv1_serialize_vw(ctx, buf, length);
...
static int tbonv1_serialize_hash_item(cton_ctx *ctx,
	cton_obj *key, cton_obj *value, size_t index, void *buf)
{
	(void) index;
	tbonv1_serialize_object(ctx, buf, key);
	tbonv1_serialize_object(ctx, buf, value);
	return 0;
}
...
static int tbonv1_serialize_hash(cton_ctx *ctx, cton_buf *buf, cton_obj *obj)
{
	...
	cton_hash_foreach(obj, (void *)buf, tbonv1_serialize_hash_item);

	return 0;
}

static int tbonv1_serialize_array_item(cton_ctx *ctx,
	cton_obj *arr_item, size_t index, void *buf)
{
	(void) index;
	tbonv1_serialize_value(ctx, buf, arr_item);

	return 0;
}

static int tbonv1_serialize_array_object(cton_ctx *ctx,
	cton_obj *arr_item, size_t index, void *buf)
{
	(void)index;
	tbonv1_serialize_object(ctx, buf, arr_item);
	return 0;
}
...
static int tbonv1_serialize_array(cton_ctx *ctx, cton_buf *buf, cton_obj *obj)
{
	...
	if (type == CTON_OBJECT) {
		return cton_array_foreach(obj,
			(void *)buf, tbonv1_serialize_array_object);
	} else {
		return cton_array_foreach(obj,
			(void *)buf, tbonv1_serialize_array_item);
	}
...
static uint8_t tbonv1_deserialize_vwlen(uint8_t *ptr)
{
	if (*ptr <= 0x7F) {
		/* 7bit, 1 byte */
		return 1;
...
static uint64_t tbonv1_deserialize_vw(uint8_t *ptr)
{
	uint64_t data;
	uint8_t  tails;
...
static uint64_t tbonv1_deserialize_64bit(uint8_t *ptr)
{
	uint64_t data;
...
static uint32_t tbonv1_deserialize_32bit(uint8_t *ptr)
{
	uint32_t data;
...
static uint16_t tbonv1_deserialize_16bit(uint8_t *ptr)
{
	uint16_t data;
...
static uint8_t tbonv1_deserialize_8bit(uint8_t *ptr)
{
	return (uint8_t)ptr[0];
}

static cton_obj *
tbonv1_deserialize_string(cton_ctx *ctx,
	size_t *index, uint8_t *ptr, size_t len, cton_type type)
{
	...
	if (*index + tbonv1_deserialize_vwlen(ptr) >= len) {
		cton_seterr(ctx, CTON_ERROR_BROKEN);
		return NULL;
	}

	str_len = tbonv1_deserialize_vw(ptr + *index);
	*index += tbonv1_deserialize_vwlen(ptr);
...
static cton_obj *
tbonv1_deserialize_hash(cton_ctx *ctx, size_t *index, uint8_t *ptr, size_t len)
{
	size_t hash_len;
	cton_obj *hash;
	cton_obj *key;
	...
	hash_len = tbonv1_deserialize_vw(ptr + *index);
	*index +=  tbonv1_deserialize_vwlen(ptr);
	...
	while (hash_len > 0) {
		key = tbonv1_deserialize_object(ctx, index, ptr, len);
		if (key == NULL) {
			return hash;
		}

		value = tbonv1_deserialize_object(ctx, index, ptr, len);
		if (value == NULL) {
...
static cton_obj *
tbonv1_deserialize_array(cton_ctx *ctx, size_t *index, uint8_t *ptr, size_t len)
{
	size_t arr_len;
	size_t arr_index;
	cton_type type;
	...
	if (*index +  tbonv1_deserialize_vwlen(ptr) + 1 >= len) {
		cton_seterr(ctx, CTON_ERROR_BROKEN);
		return NULL;
	}
	...
	*index += 1;
	...
	arr_len = tbonv1_deserialize_vw(ptr + *index);
	*index +=  tbonv1_deserialize_vwlen(ptr);
	...
	if (type == CTON_OBJECT) {
		...
		for (arr_index = 0; arr_index < arr_len; arr_index += 1) {
			obj = tbonv1_deserialize_object(ctx, index, ptr, len);
		...
		for (arr_index = 0; arr_index < arr_len; arr_index += 1) {
			...
			obj = tbonv1_deserialize_value(ctx, index, ptr, type, len);
			cton_array_set(arr, obj, arr_index);
		}
...
static cton_obj * tbonv1_deserialize_value(cton_ctx *ctx,
	size_t *index, uint8_t *ptr, cton_type type, size_t len)
{
	cton_obj *obj;
	uint8_t  *v8_ptr;
	...
	switch (type) {
		case CTON_STRING:
		case CTON_BINARY:
			obj = tbonv1_deserialize_string(ctx, index, ptr, len, type);
			break;

		case CTON_ARRAY:
			obj = tbonv1_deserialize_array(ctx, index, ptr, len);
			break;

		case CTON_HASH:
			obj = tbonv1_deserialize_hash(ctx, index, ptr, len);
			break;
		...
		case CTON_FLOAT8:
			...
			*v8_ptr = tbonv1_deserialize_8bit(ptr + *index);
			*index += 1;
			break;
		...
		case CTON_FLOAT16:
			...
			*v16_ptr = tbonv1_deserialize_16bit(ptr + *index);
			*index += 2;
			break;
		...
		case CTON_FLOAT32:
			...
			*v32_ptr = tbonv1_deserialize_32bit(ptr + *index);
			*index += 4;
			break;
		...
		case CTON_FLOAT64:
			...
			*v64_ptr = tbonv1_deserialize_64bit(ptr + *index);
			*index += 8;
			break;
...
tbonv1_deserialize_object(cton_ctx *ctx, size_t *index, uint8_t *ptr, size_t len)
{
	...
	return tbonv1_deserialize_value(ctx, index, ptr, type, len);
}
